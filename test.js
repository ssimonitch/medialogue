function minWindow(s, t) {
  ...
  while (right < s.length) {
    ...
    while (charCounter === 0) {
      ...
      /**
       * Before dropping, check if char is in targetMap. 
       * If it is, then we to increase the # occurrences for that char in the map 
       * and if the count is > 0, increase the charCounter. 
       * Meaning we no longer have our target string in the window and 
       * we need to move 'right' forward to find the next valid window 
       * (in the next iteration of the top while loop)
       */
  ...
  console.log(minWidth === Number.MAX_SAFE_INTEGER ? '' : s.substring(minLeft, minRight + 1));
}
